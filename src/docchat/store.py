...
class ChromaVectorStore(BaseStore):
    ...
    def _get_all_chunks(self) -> list[Chunk]:
        ...
        for id_, doc, meta in zip(results["ids"], results["documents"], results["metadatas"]):
            c = Chunk(text=doc, source=meta["source"], index=meta.get("index", 0), chunk_num=meta.get("chunk_num", 0), id=id_)
            chunks.append(c)
        return chunks
    ...
    def search(self, query: str, k: int = 5) -> list[SearchResult]:
        ...
        if self._bm25 is not None:
            ...
            top_indices = sorted(range(len(bm25_scores)), key=lambda i: bm25_scores[i], reverse=True)[:k_fetch]
    ...
    def _rrf(self, results_list: list[list[SearchResult]], k_rrf: int = 60, k_out: int = 20) -> list[SearchResult]:
